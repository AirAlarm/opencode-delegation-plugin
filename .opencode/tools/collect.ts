...
export default tool({
  description:
    "Collect the result of a completed delegation task. Updates the ledger status with the worker's result summary " +
    "and rubric evaluation. Use this after a worker session finishes (session.idle event). " +
    "Extract the result summary from the worker session's output and pass it as the result_summary argument.",
  args: {
  ...
  async execute(args, context) {
    ...
    if (task.status !== "dispatched" && task.status !== "running") {
      return JSON.stringify({
        error: `Task ${args.task_id} is in status '${task.status}'. Only 'dispatched' or 'running' tasks can be collected.`,
      })
    }
