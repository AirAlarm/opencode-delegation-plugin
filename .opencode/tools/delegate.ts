...
export default tool({
  ...
  async execute(args, context) {
    ...
    // Find first available lane that isn't busy or rate-limited
    let agent = lanes[0]
    let reason = ""
    ...
    for (const lane of lanes) {
      ...
      if (!busyAgents.has(lane)) {
        agent = lane
        break
      }
    }
    
    // If all lanes are rate-limited or busy, use the first lane and note the issue
    const allRateLimited = lanes.every((l) => isProviderRateLimited(rateLimits, AGENT_PROVIDER[lane]))
    const allBusy = lanes.every((l) => busyAgents.has(l))
    
    if (allRateLimited) {
      reason = `All ${args.task_class} lanes rate-limited (${[...rateLimitedProviders].join(", ")}). Assigned to ${agent} but will likely fail. Use mark_rate_limited_clear to clear limits when refreshed.`
    } else if (allBusy) {
      reason = `All ${args.task_class} lanes busy. Assigned to ${agent} (queued).`
    } else {
      const availableLanes = lanes.filter((l) => !busyAgents.has(l) && !isProviderRateLimited(rateLimits, AGENT_PROVIDER[lane]))
      reason = availableLanes.length === lanes.length
        ? `Task ${id} created. Dispatch to @${agent}.`
        : `Task ${id} created. Dispatch to @${agent}. (Some lanes unavailable: ${[...rateLimitedProviders].join(", ")} rate-limited)`
    }
